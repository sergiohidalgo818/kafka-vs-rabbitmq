...
### Measures
The key measure that it is used to analyze both platforms is the `size_msg` parameter (the number of characters in a string message). This measure is complemented with `incremental` option, wich makes messages follow the size $max((sizemsg*list_i/sizelist), 1)$ where $list_i$ is the position in the data list, and $sizelist$ is the size of that same list. In this way, instead of having all messages the same size, they have an **incremental** size (hence its name).
